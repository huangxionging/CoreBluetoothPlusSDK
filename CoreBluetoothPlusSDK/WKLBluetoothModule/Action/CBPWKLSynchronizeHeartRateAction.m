//
//  CBPWKLSynchronizeHeartRateAction.m
...
@interface CBPWKLSynchronizeHeartRateAction () {
    ...
    //一天的分时数据
    NSMutableArray *_oneDayDataArray;
}
 ...
 * 包含所有长包数据
 */
@property (nonatomic, strong) NSMutableArray *allDayData;

@end

@implementation CBPWKLSynchronizeHeartRateAction
...
+ (NSSet *)keySetForAction {
    return [NSSet setWithObjects:@"0x20", @"0x05", @"0x06", nil];
}
...
- (NSMutableArray *)allDayData {
    if (_allDayData == nil) {
        _allDayData = [NSMutableArray arrayWithCapacity: 10];
    }
    return _allDayData;
}
...
- (NSMutableDictionary *)dataDiction {
    if (_dataDiction == nil) {
        _dataDiction = [NSMutableDictionary dictionaryWithCapacity: 10];
    }
    return _dataDiction;
}
...
- (void) handleFirstAnswer: (Byte *) bytes {
    ...
    [self.allDayData removeAllObjects];
    
    [self.dataDiction setObject: totalCount forKey: @"total_count"];
...
- (void) handleShortPackageActionAnswer: (Byte *) bytes {
    ...
    _oneDayDataArray = [NSMutableArray arrayWithCapacity: 10];
...
//    NSInteger time = timeInterval.integerValue;
    for (NSInteger index = 0; index < _longPackageData.length; index += 3) {
        
        NSInteger timeMinute = (bytes[index] << 8) + bytes[index + 1];
        // 时间段
        NSString *timeSection = [NSString stringWithFormat:@"%02ld:%02ld", (long) timeMinute / 60, (long) timeMinute % 60];
        
        NSInteger hearteRateData = bytes[index + 2];
        
        NSMutableDictionary *diction = [NSMutableDictionary dictionaryWithCapacity: 2];
        
        if (hearteRateData >= 40 && hearteRateData <= 220) {
            NSString *hearteRateDatas = [NSString stringWithFormat: @"%ld", (long)hearteRateData];
            
            // 心率值
            [diction setObject: hearteRateDatas forKey: @"heart_rate_value"];
            // 时间
            [diction setObject: timeSection forKey: @"time"];
        } else {
            // 步数
            [diction setObject: @"0" forKey: @"heart_rate_value"];
            // 时间段
            [diction setObject: timeSection forKey: @"time"];
        }
        
        // 添加一条数据
        [_oneDayDataArray addObject: diction];
    }
    
    [_oneDayDataDict setObject: _oneDayDataArray forKey: @"heart_rate_data"];
    
    // 添加一天的数据
    [self.allDayData addObject: _oneDayDataDict];
}
...
- (void) sendBitControllTable {
    ...
    NSLog(@"发送的位域表: %@", bitControlTableData);
...
- (void) handleNextDayDataPackage {
    ...
    if (_longPackageSerialNumber == _dayCount || _dayCount == 1) {
        // 表示所有数据发送完成
        // 计步数据
        [self.dataDiction setObject: self.allDayData forKey: @"all_day_heart_rate_data"];
        
        // 回复数据
        //  完成
        [[CBPDispatchMessageManager shareManager] dispatchTarget: self method: @"callBackResult:", self.dataDiction, nil];
        return;
    }
...
- (void)timeOut {
    CBPBaseError *baseError = [CBPBaseError errorWithcode:kBaseErrorTypeSynchronizeDataTimeOut info: @"同步计步数据超时"];
