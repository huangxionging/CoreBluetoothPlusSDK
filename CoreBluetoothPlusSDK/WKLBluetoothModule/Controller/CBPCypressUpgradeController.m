//
//  CBPCypressUpgradeController.m
//  CoreBluetoothPlusSDK
//
//  Created by huangxiong on 2016/10/20.
//  Copyright © 2016年 huangxiong. All rights reserved.
//

#import "CBPCypressUpgradeController.h"
...
@interface CBPCypressUpgradeController ()
@property (nonatomic, strong) CBPBasePeripheralModel *peripheralModel;


@property (nonatomic, strong) CBPWKLCypressUpgradeServiceCharacteristicManager *serviceCharacteristicManager;
@end

@implementation CBPCypressUpgradeController
...
- (instancetype)init {
    ...
    if (self) {
        ...
        // 单例
        self.serviceCharacteristicManager =[CBPWKLCypressUpgradeServiceCharacteristicManager shareManager];
...
- (void)sendAction:(NSString *)actionName parameters:(id)parameters progress:(void (^)(id))progress success:(void (^)(CBPBaseAction *, id))success failure:(void (^)(CBPBaseAction *, CBPBaseError *))failure {
    ...
    __weak CBPCypressUpgradeController *weakSelf = self;
    // 只需要一次
    ...
    // 保持 action
    [self.actionSheet setObject: action forKey: @"cypress_upgrade"];
    ...
    void (^changeDeviceServiceSingal)() = ^(){
        ...
        [weakSelf.baseDevice setDiscoverServiceBlock:^NSArray<CBUUID *> *(CBPBaseServiceModel *discoverServiceModel) {
            ...
            // 这下便可保证只发现该服务的特征
            CBUUID *UUID = weakSelf.serviceCharacteristicManager.serviceCharacteristicModel.discoverServiceUUID;
        ...
        [weakSelf.baseDevice setDiscoverServiceCharacteristicBlock:^(CBPBaseServiceModel *discoverServiceCharacteristicModel, CBPBasePeripheralModel *peripheralModel) {
            ...
            for (CBCharacteristic *cha in discoverServiceCharacteristicModel.service.characteristics) {
                ...
                // 只有这个特征是通知特征
                if ([cha.UUID isEqual: weakSelf.serviceCharacteristicManager.serviceCharacteristicModel.notifyCharacteristicUUID]) {
            ...
            // 查询 action
            CBPBaseAction *action = [weakSelf.actionSheet objectForKey: @"cypress_upgrade"];
            // 调整数据
...
- (void) reciveUpdateData {
    ...
    [weakSelf.baseDevice setUpdateDataBlock:^(CBPBaseActionDataModel *actionDataModel) {
        ...
        CBPWKLCypressUpgradeServiceCharacteristicModel *model = [weakSelf.serviceCharacteristicManager serviceCharacteristicModel];
        // 等于
                    // 或者投送
